function render(){const focusToken=captureFocusToken();let view;if(route==="home")view=homeView();else if(route==="quick")view=quickView();else if(route==="assessment-intro")view=assessmentIntroView();else if(route==="assessment")view=assessmentView();else if(route==="assessment-phase-complete")view=assessmentPhaseCompleteView(currentFlow?.completedPhase??"pre");else if(route==="dashboard")view=dashboardView();else if(route==="emergency")view=emergencyView();else if(route==="data")view=dataView();else view=homeView();app.innerHTML=view;bindEvents();restoreFocusToken(focusToken);}
function captureFocusToken(){const element=document.activeElement;if(!element||!app.contains(element))return null;for(const attribute of ["data-select-action","data-select-signal","data-assessment-action","data-assessment-judgment","data-assessment-signal","data-assessment-confidence","data-emergency-request","data-emergency-official"]){if(element.hasAttribute(attribute))return {attribute,value:element.getAttribute(attribute)};}return null;}
function restoreFocusToken(token){if(!token)return;const target=document.querySelector(`[${token.attribute}="${token.value}"]`);target?.focus({preventScroll:true});}
function bindEvents(){
