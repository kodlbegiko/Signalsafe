...
function toast(message, type = "info") { clearTimeout(toastTimer); let node = document.querySelector("#toast"); if (!node) { node = document.createElement("div"); node.id = "toast"; document.body.append(node); } node.setAttribute("role", type === "error" ? "alert" : "status"); node.setAttribute("aria-live", type === "error" ? "assertive" : "polite"); node.setAttribute("aria-atomic", "true"); node.className = `toast toast--${type} is-visible`; node.textContent = message; toastTimer = setTimeout(() => node.classList.remove("is-visible"), 2800); }
